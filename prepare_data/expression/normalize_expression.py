...
def get_tissue_sample_ids(tissue_sample_ids_file):
    """
    Extracts donors from premade file
    """
    samples = []
    with open(tissue_sample_ids_file) as fh:
        for line in fh:
...
def read_gct(gct_file, donor_ids):
    ...
    """    
    df = pd.read_csv(gct_file, sep='\t', skiprows=2, usecols=['Name'] + donor_ids, index_col=0)
    #df.drop('Description', axis=1, inplace=True)
    #df.index.name = 'gene_id'
    return df#[[i for i in df.columns if i in donor_ids]]
...
if __name__=='__main__':
    ...
    print("Getting donor ids...")
    donor_ids = get_tissue_sample_ids(args.donors)
    print("{} donors found".format(len(donor_ids)))
    print("Reading rna expression data...")
    ...
    print("Writing unsorted expression to file")
